            ...
            for (let i = 0; i < 6; i++) {
                ...
                for (let j = 0; j < 7; j++) {
                    if ((i === 0 && j < firstDay.getDay()) || date > lastDay.getDate()) {
                        ...
                        if (selectedMonth === months[todays.getMonth()] && selectedYear === todays.getFullYear() &&
                            date === todays.getDate()) {
                            td.addClass("todays");
                        }
                        row.append(td);
                ...
                for (let i = 0; i < 6; i++) {
                    ...
                    for (let j = 0; j < 7; j++) {
                        if ((i === 0 && j < firstDay.getDay()) || date > lastDay.getDate()) {
                            ...
                            let td = $(`<td class="d" id="${date}" data-day="${date}">`).text(date);
                            if (selectedMonth === months[today.getMonth()] && selectedYear === today.getFullYear() &&
                                date === today.getDate()) {
                                td.addClass("today");
                            }
                            row.append(td);
...
?>
    ...
    <div class="webview_progressdetails">
        ...
        <div class="detailed_progress_container2" id="container3" style="display:none">
            <?php
                if(count($off) == 0){
                    echo("<h5>Set Goal To Fetch Data Here</h5>");
                }
            foreach ($off as $r) {
                ...
                echo('<div class="stats-col">');
                if ($r['steps'] != '-1') {
                    echo ('<div class="info"><span>Steps</span><div class="symbols" style="display: flex;flex-wrap: wrap;" ><div><img class="stat-bar" src="'.$DEFAULT_PATH.'assets/images/orange.svg" alt=""></div><div style="margin-top:0.1rem"><span style="margin-left:0.5rem">' . $r["steps"] . ' steps</span></div></div></div>');
                }
                if ($r['heart'] != '-1') {
                    echo ('<div class="info"><span>Heart Rate</span> <div class="symbols"><div><img class="stat-bar" src="'.$DEFAULT_PATH.'assets/images/pink.svg" alt=""></div><div style="margin-top:0.1rem"><span style="margin-left:0.5rem">' . $r["heart"] . ' bpm</span></div></div></div>');
                }
    ...
    <div class="mobview_progressdetails">
        ...
        <div class="mobview_container1" id="container4">
            <?php
                ...
                ?>
                    <?php
                    if ($r['steps'] != '-1' || $r['heart'] != '-1' || $r['weight'] != '-1' || $r['sleep'] != '-1') {
                        echo('<div class="mob_container1_wrapper1">');
        ...
        <div class="mobview_container1" id="container5" style="display:none">
                    <?php
                        foreach ($off as $rb) {
                    ?>
