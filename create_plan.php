...
if (isset($_POST['final_save_btn'])) {
    ...
    if (isset($_FILES["image"]) && $_FILES["image"]["error"] == 0) {
        ...
        $randomNumber = mt_rand(1000, 9999); // Using mt_rand() to generate a random number with 4 digits
        $imageFileNameWithRandom = $randomNumber . '_' . $imageFileName;

        $imgPath = 'uploads/' . $imageFileNameWithRandom;
        move_uploaded_file($image, $imgPath);
    }
    ...
    $sql1 = "INSERT INTO `create_plan` (`dietitianuserID`,`image`,`name`,`tags`, `start_date`, `end_date`,`features`, `description`, `price`) VALUES ('$dietitianuserID','$imageFileNameWithRandom','$planname','$tags','$start_date','$end_date','$features','$description','$price')";
    echo $sql1;
...
<style>
    ...
    * {
    ...
    #imagePreview.selected-image {
        ...
        /* Add your custom styles here for the selected image */
        border: 2px solid #7282FB;
        box-shadow: 0 0 5px rgba(114, 130, 251, 0.7);
    }
...
<body>
        ...
        <form action="create_plan.php" method="post" enctype="multipart/form-data">
            ...
            <div class="plan_form">
                ...
                <div class="event-image">
                    <input type="file" id="fileInput" accept="image/*" onchange="showImagePreview(event)" name="image" style="position:absolute;width: 144px; height:135px; opacity:0; cursor:pointer;" title="">
                    <div class="ev-img" style="position:relative">

                        <div style="background: #7282FB;border-radius:100px;padding:0.5rem;margin-top:3rem;cursor:pointer;" id="whitePencilIcon" onclick="handleWhitePencilClick(event)">
                            <img src="<?= $DEFAULT_PATH ?>assets/images/whitepencil.svg" alt="">
                        </div>
                        <img id="imagePreview" src="<?= $DEFAULT_PATH ?>assets/images/create_edit_plan.svg" class="edit-plan-img" style="position:absolute;width:15rem;margin-top:2rem; z-index:-2; width: 144px;
                    height: 110px;">
                    </div>
                </div>
            </div>
                ...
                <div class="evt-form">
                    ...
                    <div class="tags" style="margin-top:1rem">
                        ...
                        <div class="tag mt-3" style="width:100%;">
                            <div style=" height: 40px; display: inline-block; border:none;font-size: 12px; width: 80%; background: white !important;
                                        border-radius: 7px !important;
                                        /* border: 1px solid black !important; */
                                        box-shadow: 0px 1.7px 5px rgba(0, 0, 0, 0.25) !important;">
                                <select id="fields_u" style="position: relative;" name="languages[]" multiple required>
                            ...
                            <button onclick="showPopup()" style="float:right;display: inline-block;width:40px; height: 40px; border:none; text-align: center;" class="tag-item openButton openBtn"><i class="fas fa-plus" style="font-size:25px;" class="plus-icon"></i></button>
                            <div class="popup">
    ...
    " required />
                    ...
                    <div class="features_main mt-2">
                        <input type="hidden" id="dynamic-features" name="text_arr[]">
    ...
    padding:1.5rem;" required />
                    ...
                    <!-------------------------------------BOTTON BUTTONS--------------------------------------------->
                    <div class="btns">
                        <button style="color:#7282FB" onclick="window.location.href='<?= $DEFAULT_PATH ?>myplan.php'">Cancel</button>
...
<script>
    ...
    function addSkill(skill) {
        ...
        skillInput.value = "";
        modal.style.display = "none";
        document.getElementById("dynamic-features").value = skills.join(" , ");
    }
...
<script>
    function handleWhitePencilClick(event) {
        // Clicking the transparent overlay
    ...
    function showImagePreview(event) {
        ...
        if (input.files && input.files[0]) {
            ...
            reader.onload = function(e) {
                ...
                document.getElementById('imagePreview').classList.add('selected-image');
                document.getElementById('whitePencilIcon').style.display = 'none';
            };
            reader.readAsDataURL(input.files[0]);
        } else {
            // If no image is selected, show the default "create_edit_plan.svg" image
            document.getElementById('imagePreview').style.display = 'block';
            document.getElementById('imagePreview').classList.remove('selected-image');
            document.getElementById('imagePreview').src = '<?= $DEFAULT_PATH ?>assets/images/create_edit_plan.svg';
            document.getElementById('whitePencilIcon').style.display = 'block';
        }
    }
