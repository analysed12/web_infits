...
<head>
    ...
    <title>Infits | Recipes_Snacks</title>
...
<body>
    <br>
    <?php
    unset($_SESSION['snack_recipe_search']);
    if (isset($_POST['search_btn'])) {
        if (!empty($_POST['search_query'])) {
            $_SESSION['snack_recipe_search'] = $_POST['search_query'];
            $snackRecipeSearch = $_POST['search_query'];
        }
    } ?>
    <div class="header">
        <div class="heading" style="display:flex;justify-content:space-between;margin-top:-5px">
            ...
            <h4 style="margin-left:30px;color:#787885;font-size: 23px;margin-top:1.8rem;">Snacks</h4>
        </div>
        <div class="search" style="margin-right:5rem;display:flex;gap:1.5rem">
            <div class="searchbox">
                <form method="post">
                    <button style="background-color:white;border:none;" id="search-btn" name="search_btn"><img src="<?= $DEFAULT_PATH ?>assets/images/search1.svg" alt=""></button>
                    <input type="search" name="search_query" placeholder="Search" style="outline:none;border:none;font-size:20px;margin-left:1rem;width:200px;" value="<?php if (!empty($snackRecipeSearch)) {
                                                                                                                                                                            echo $snackRecipeSearch;
                                                                                                                                                                        } ?>">
                </form>
            </div>
        </div>
    ...
    <div class="container1" id="myDIV1" onscroll="myFunction()">
        <a href="all_recipes.php" style="color: inherit;" class="breakfast" id="btn1">
            <div class="top-card" style=" background-color: #85B4A1;margin-left:50px;width:320px; height:211px;">
        ...
        <a href="all_recipes.php" style="color: inherit;" class="lunches" id="btn2">
            <div class="top-card" style=" background-color: #aea2d8;width:320px; height:211px;">
        ...
        <a href=" all_recipes.php" style="color: inherit;" class="snacks" id="btn3">
                <div class="top-card" style=" background-color: #6cb7d9;width:320px; height:211px;">
    ...
    <div class="middle_wrapper" style="display:flex;justify-content:space-between;margin-top:20px;margin-left:5rem;margin-right:2.5rem">
        ...
        <a href="recipe_all_snacks.php" style="text-decoration:none;">
            <h3 style="color:#6A6A6A;font-size:20px; margin-right: 50px;">View All</h3>
        </a>
    </div>

    <!-- recipes from db -->
    <?php
    $sql = "SELECT * FROM `dietitian_recipes` WHERE dietitian_id = '{$_SESSION['dietitian_id']}' AND recipe_courses = 'snacks'";
    $sql2 = "SELECT dr.* FROM `default_recipes` dr LEFT JOIN `updated_by_users` ubu ON dr.`drecipe_id` = ubu.`updated_drecipe_id`AND ubu.`dietitian_id`='{$_SESSION['dietitian_id']}' WHERE ubu.`updated_drecipe_id` IS NULL AND ubu.`dietitian_id` IS NULL AND drecipe_course = 'snacks'";
    if (!empty($snackRecipeSearch)) {
        $sql .= " AND recipe_name LIKE '%$snackRecipeSearch%'";
        $sql2 .= " AND drecipe_name LIKE '%$snackRecipeSearch%'";
    }
    ...
    <div class="flex row">
            ...
            <div class="card d-flex" style="padding:15px; width:310px; height:238px;border-radius:16px; margin:35px 35px;">
                ...
                <div class="img-dis" style="width:100%; text-align:center;">
                    <?php if ($d['recipe_img'] != "") {
                        ...
                        $imgSrc = $DEFAULT_PATH . "assets/images/snacks.svg";
                    } ?>
                    <img src="<?= $imgSrc ?>" style="margin-top:-5px;height:115px; width:160px; margin-left:15px; object-fit:cover;" />
                </div>
        ...
        ?>
            <div class="card d-flex" style="padding:15px; width:310px; height:238px;border-radius:16px; margin:35px 35px ;">
                ...
                <div class="img-dis" style="width:100%; text-align:center;">
                    <img src="<?= $DEFAULT_PATH ?>assets/images/snacks.svg" style="margin-top:-5px;height:115px; width:160px; margin-left:15px; object-fit:cover;" />
                </div>
                <div class="d-flex justify-content-between">
        ...
        <?php } ?>
        <a class="butt" href="create_recipe.php" style="border-radius:50%;background-color:#9C74F5;width:85px;height:85px;filter: drop-shadow(0px 0px 68px rgba(0, 0, 0, 0.3));color:white;font-size:60px;border:none;position:absolute;right:50px;bottom:-130px;display:flex;justify-content:center;align-items:center;text-decoration:none;">+</a>
    </div>
        ...
        if (popholder.style.display!="block"){
            ...
            if (typeof stuff === 'object' && typeof dstuff==='string'){
                ...
                if (stuff['recipe_img']!=="" && stuff['recipe_img']!==null){
                    ...
                    hover_img.src = "<?=$DEFAULT_PATH?>assets/images/snacks.svg"
                }
